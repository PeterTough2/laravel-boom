...
if (!function_exists('json_decode')) {
    echo ('We could not find json_decode. json_decode is found in php 5.2 and up, but may be missing on some Linux systems due to licensing conflicts. If you are running ubuntu try "sudo apt-get install php5-json".');
}
...
class Vimeo
{
    ...
    public function upload($file_path, $upgrade_to_1080 = false, $machine_id = null)
    {
        ...
        if (!is_file($file_path)) {
            echo ('Unable to locate file to upload.');
        }
    ...
    public function uploadImage($pictures_uri, $file_path, $activate = false)
    {
        ...
        if ($pictures_response['status'] !== 201) {
            echo ('Unable to request an upload url from vimeo');
        }
    ...
    private function perform_upload($file_path, $ticket)
    {
        if ($ticket['status'] !== 201) {
            ...
            echo ('Unable to get an upload ticket.' . $ticket_error);
        }
        ...
        if ($status !== 201) {
            ...
            echo ('Error completing the upload.'. $error);
        }
